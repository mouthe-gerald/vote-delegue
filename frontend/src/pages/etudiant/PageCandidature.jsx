...
const PageCandidature = () => {
  ...
  const statutConfig = {
    ...
    RETIREE:         { color: 'bg-slate-500/15 text-slate-500 border-slate-500/20',   msg: 'Candidature retirée.' },
  }
  ...
  return (
    <div className="min-h-screen bg-white shadow-sm">
      ...
      {/* Header */}
      <header className="bg-slate-800 border-b border-slate-200 px-4 sm:px-6 h-14 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-slate-700 font-bold text-sm">Ma Candidature</h1>
            <p className="text-slate-400 text-xs">{election?.titre || 'Aucune élection active'}</p>
          </div>
        </div>
      </header>
        ...
        {candidature ? (
            ...
            <div className="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden">
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-1 h-5 bg-amber-500 rounded-full" />
                  <h3 className="text-slate-700 font-bold text-sm">Programme Électoral</h3>
                </div>
          ...
          <div className="bg-white shadow-sm border border-slate-200 rounded-xl p-6">
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-bold text-slate-700 mb-1">Déposer ma candidature</h2>
            ...
            {!election ? (
              <div className="text-center py-6">
                ...
                <p className="text-slate-500 text-sm">Aucune élection ouverte aux candidatures.</p>
              </div>
            ) : (
              <form onSubmit={handleSoumettre} className="flex flex-col gap-5">
                ...
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-700 font-bold text-sm">{user?.prenom} {user?.nom}</p>
                    <p className="text-slate-500 text-xs">{user?.profil?.filiere} — {user?.profil?.niveau}</p>
                  </div>
                </div>

                {/* Photo */}
                <div>
                  <label className="text-slate-500 text-xs font-medium mb-2 flex items-center gap-1.5">
                    <Camera size={13} /> Photo de campagne (optionnelle)
                  </label>
                  {photoPreview && (
                    <img src={photoPreview} alt="Aperçu"
                      className="w-24 h-24 rounded-xl object-cover border border-slate-200 mb-3" />
                  )}
                ...
                <div>
                  ...
                  {/* Import fichier */}
                  <div className="mb-3 p-3 bg-white shadow-sm border border-slate-200 rounded-xl">
                    <p className="text-slate-500 text-xs mb-2">📎 Importer un fichier (PDF ou Word)</p>
                  ...
                  <p className="text-slate-400 text-xs mb-2">ou écrire directement :</p>
                  <textarea value={programme} onChange={e => setProgramme(e.target.value)}
                    placeholder="Décrivez votre programme électoral..."
                    className="w-full px-4 py-3 rounded-xl text-sm text-slate-700 bg-slate-800 border border-slate-200 outline-none resize-none h-40 placeholder-gray-400 focus:border-amber-500/50 transition-colors" />
                  <p className="text-slate-500 text-xs mt-1">{programme.length} caractères</p>
                </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white shadow-sm border border-slate-200 rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-slate-700 font-bold">Demande de retrait</h3>
                <p className="text-slate-500 text-xs">Soumise à l'administrateur pour approbation.</p>
              </div>
            </div>
            <textarea value={motifRetrait} onChange={e => setMotifRetrait(e.target.value)}
              placeholder="Expliquez votre motif..."
              className="w-full px-4 py-3 rounded-xl text-sm text-slate-700 bg-slate-800 border border-slate-200 outline-none resize-none h-28 mb-4 placeholder-gray-400" />

      ...
      <header className="bg-slate-950 border-b border-white/5 px-4 sm:px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-white font-bold text-sm">Gestion des Utilisateurs</h1>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          <button onClick={() => setOnglet('actifs')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              onglet === 'actifs' ? 'bg-amber-500 text-slate-900' : 'bg-white/5 text-slate-400 hover:bg-white/10 border border-white/10'
            }`}>
            Actifs ({actifs.length})
          </button>
          <button onClick={() => setOnglet('corbeille')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              onglet === 'corbeille' ? 'bg-red-500 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10 border border-white/10'
            }`}>
          ...
          {['', 'ETUDIANT', 'CANDIDAT'].map((r, i) => (
            <button key={i} onClick={() => setFiltreRole(r)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                filtreRole === r ? 'bg-blue-600 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10 border border-white/10'
              }`}>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="text-left px-5 py-3.5 text-slate-500 text-xs font-semibold hidden sm:table-cell">Filière / Niveau</th>
                  <th className="text-left px-5 py-3.5 text-slate-500 text-xs font-semibold">Rôle</th>
                  <th className="text-left px-5 py-3.5 text-slate-500 text-xs font-semibold hidden md:table-cell">Statut</th>
                  <th className="text-left px-5 py-3.5 text-slate-500 text-xs font-semibold">Actions</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="text-center py-12">
                    ...
                    <p className="text-slate-500 text-sm">{onglet === 'corbeille' ? 'La corbeille est vide' : 'Aucun utilisateur trouvé'}</p>
                  </td></tr>
                ) : liste.map(u => (
                  <tr key={u.id} className="border-t border-white/5 hover:bg-white/3 transition-colors">
                    <td className="px-5 py-3.5 text-slate-300 text-xs font-mono">{u.matricule}</td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-white/10 rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-white font-bold">Modifier l'utilisateur</h3>
              <button onClick={() => setShowEdit(false)} className="text-slate-400 hover:text-white"><X size={18} /></button>
            </div>
                ...
                <>
                  <div>
                    ...
                    <select value={form.filiere} onChange={e => setForm({...form, filiere: e.target.value})}
                      className="w-full px-4 py-3 rounded-xl text-sm text-white bg-slate-700 border border-white/10 outline-none">

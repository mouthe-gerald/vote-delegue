...
const Inscription = () => {
  ...
  const handleVisage = async () => {
    ...
    setErreur('')
    try {
      // Connecter d'abord pour avoir le token
      ...
      await authAPI.encoderVisage({ image_base64: base64 })
      toast.success('Inscription complète ! Vous pouvez vous connecter.')
      navigate('/connexion')
    } catch (err) {
      setErreur(err.response?.data?.erreur || 'Erreur lors de la capture.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8">
        ...
        {etape === 0 && (
          <form onSubmit={handleInscription} className="space-y-4">
            ...
            <div className="relative">
              ...
              <input placeholder="Matricule" value={form.matricule}
        ...
        {etape === 1 && (
          <div className="text-center space-y-6">
            ...
            <button onClick={handleOTP} disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
              {loading
                ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                : 'Vérifier le code'}
            </button>
        ...
        {etape === 2 && (
          <div className="space-y-5">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm mt-1">
                Positionnez votre visage dans le cadre et capturez
              </p>
            </div>
            ...
            <div className="flex gap-3">
              {visageCapture ? (
                <>
                  <button onClick={() => setVisageCapture(null)}
